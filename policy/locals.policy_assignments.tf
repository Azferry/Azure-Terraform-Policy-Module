...
# Merge the Policy Assignment maps into a single map. If duplicates exist due to a custom Policy Assignment being
# defined to override a built-in definition, this is handled by merging the custom policies after the built-in policies.
locals {
...
# Generate a map of parameters from the archetype definition and merge with the parameters 
# provided using var.parameters. Used to determine the parameter values for Policy Assignments.
locals {
...
locals {
  archetype_policy_assignments_output = [
    for k, policy_assignment in local.archetype_policy_assignments_map :
    {
      #   resource_id = "${local.provider_path.policy_assignment}${policy_assignment}"
      scope_id = local.scope_id
      template = local.archetype_policy_assignments_map[k]

      # Also need to generate a set of parameters for each Policy Assignment if provided as part of the parameters
      # variable. These come from the archetype_config object in the enterprise_scale module and are merged with 
      # the Policy Assignment template values to provide overrides.
      parameters = merge(
      ...
      # The following attribute is used to control the enforcement mode on Policy Assignments, allowing the 
      # template values to be over-written with dynamic values generated by the module, or default to true 
      # when neither exist (as per default platform behaviour).
